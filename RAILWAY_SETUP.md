...
### 原因
Railway 每次重新部署都会创建全新的容器，容器内的文件系统是临时的。`data/history/` 和 `data/output/` 目录中的数据会在重新部署时丢失。

### 解决方案：配置持久化卷（Volume）

**⚠️ Railway 单项目只支持挂载一个卷，因此我们将所有数据统一存储在 `/app/data` 目录下。**
...
| 目录 | 用途 | 实际路径 | 建议大小 |
|------|------|---------|---------|
| `/app/data` | 所有持久化数据 | Volume 挂载点 | 5-10GB |
| `/app/data/history` | 历史记录（JSON 文件） | 自动创建 | - |
| `/app/data/output` | 生成的图片文件 | 自动创建 | - |

**注意**：`history` 和 `output` 子目录会由应用自动创建，无需手动创建。
...
5. **单卷限制**：Railway 单项目只能挂载一个卷，因此我们将所有数据统一存储在 `/app/data` 下。
...
2. 确认 Dockerfile 中创建了 `data/history` 和 `data/output` 目录
3. 检查 Volume 挂载路径是否为 `/app/data`
4. 联系 Railway 支持
